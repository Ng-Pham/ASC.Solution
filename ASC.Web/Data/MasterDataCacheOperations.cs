    ...
    public class MasterDataCacheOperations : IMasterDataCacheOperations
    {
        ...
        public async Task CreateMasterDataCacheAsync()
        {
            var masterDataCache = new MasterDataCache
            {
                ...
                Values = (await _masterData.GetAllMasterValueAsync()).Where(p => p.IsActive == true).ToList()
            };
        ...
        {
            //await _cache.RemoveAsync("MasterDataCache");
            //await CreateMasterDataCacheAsync(); // gọi lại để rebuild cache
